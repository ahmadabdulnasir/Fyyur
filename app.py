...
class Artist(db.Model):
    ...
    state = db.Column(db.String(120))
    phone = db.Column(db.String(120))
    # Genre have its own table, with a many2many relationship with Artist
...
class Show(db.Model):
    ...
    def __repr__(self):
        return f'<Show {self.start_time} artist_id={self.artist_id} venue_id={self.venue_id}>'
...
def create_venue_submission():
  ...
  if not form.validate():
      ...
      try:
          ...
          # genres can't take a list of strings, it needs to be db objects
          ...
          print(f'Exception "{exp}" in create_venue_submission()')
          db.session.rollback()
      finally:
          db.session.close()
...
def delete_venue(venue_id):
  ...
  if not venue:
      ...
      # save the vanue name for response message
...
def show_artist(artist_id):
  ...
  print(artist)
  if not artist:
      # Artist not found, Redirect home
      return redirect(url_for('index'))
  else:
...
def edit_artist_submission(artist_id):
  ...
  if not form.validate():
      ...
      try:
          ...
          print(f'Exception "{exp}" in edit_artist_submission()')
          db.session.rollback()
      finally:
          db.session.close()
...
def edit_venue_submission(venue_id):
  ...
  if not form.validate():
      ...
      try:
          ...
          error_in_update = True
          print(f'Exception "{exp}" in edit_venue_submission()')
          db.session.rollback()
      finally:
          db.session.close()
      if not error_in_update:
...
def create_show_submission():
  ...
  try:
      ...
      error_in_insert = True
      print(f'Exception "{exp}" in create_show_submission()')
      db.session.rollback()
  finally:
      db.session.close()
  if error_in_insert:
